...
test("uses ACERVO and ULife domains by default", () => {
  assert.deepEqual(resolveInstitutionalEmailDomains({}), DEFAULT_INSTITUTIONAL_EMAIL_DOMAINS);
});

test("keeps the legacy domain configuration and adds ULife support", () => {
...
test("accepts only exact institutional domains", () => {
  const domains = parseInstitutionalEmailDomains("acervo.edu,ulife.com.br");
...
test("normalizes addresses and formats the allowed domain message", () => {
  ...
  assert.equal(
    formatInstitutionalEmailDomains(["acervo.edu", "ulife.com.br"]),
    "@acervo.edu ou @ulife.com.br",
  );
});
